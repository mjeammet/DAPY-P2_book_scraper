...
if response.ok:
    ...
    for category in category_list[0:2]: # sublist the list for testing purposes
        ...
        for page in range(2,nb_pages_in_category+1):
            page_url = index_url + category['href'].replace('index.html', 'page-' + str(page) + '.html')
            category_page.append(BeautifulSoup(requests.get(page_url).text, 'html.parser'))
